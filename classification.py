...
class KClassification(object):
    ...
    def train(self,train_dir=None):
        ...
        lr = tf.train.exponential_decay(ini_lr, global_step,decay_steps ,decay_rate,staircase=True)
        ...
        try:
            while not coord.should_stop():
                num_run += 1
                if num_run % 10 ==0:
                    _,loss_value,sum_up = self.sess.run([train_op,loss,summary_op])
                    self.prev_num += 1
    ...
    def classify(self,image_path):
        ...
        try:
            ...
            softmax_value = self.sess.run(tf.contrib.layers.softmax(self.logit),feed_dict={self.image_input: image_value})
    ...
    def prepare_classify(self):
        ...
        self.image_input = tf.placeholder(tf.float32,shape=[self.crop_size[0],self.crop_size[1],3])
...
class _LoggerHook(tf.train.SessionRunHook):
    ...
    def before_run(self, run_context):
        self._step += 1
        return tf.train.SessionRunArgs([self.loss,self.lr])  # Asks for loss value.
